...
$sql = "SELECT * FROM food_latest WHERE H_Name='".$name."'";
...
while($row = mysqli_fetch_assoc($result)){
	...
	$italian = $row["Italian"];
	$spanish = $row["Spanish"];
	$mediterranean = $row["Mediterranean"];
	$mexican = $row["Mexican"];
	$thai = $row["Thai"];
	$continental = $row["Continental"];
	$tibetan = $row["Tibetan"];
	$southindian = $row["SouthIndian"];
	$streetfood = $row["Streetfood"];
	$gujarati = $row["Gujarati"];
	$lucknowi = $row["Lucknowi"];
...
$result2=mysqli_query($con,"select * from rating where H_Id='".$H_Id."'");
$row2=mysqli_fetch_assoc($result2);
$rating=intval($row2["avgRating"]);
$NoofUserRated=$row2["NoofUserRated"];

$result3=mysqli_query($con,"select * from review_food where ID='".$H_Id."'");
...
$totalscore=($row3["Food"]+$row3["Staff"]+$row3["Value"]+$row3["Atmosphere"])/4;
            ...
            <div class='review-content-wrapper'>
                ...
                <div class='review-rating'>
                    <dl>
                        <dt>Food</dt>
                        <dd>";
				for($i=0;$i<$row3['Food'];$i++){
					echo "<i class='fa fa-star'></i>";
				}
echo"                        </dd>
                        <dt>Staff</dt>
                        <dd>";
				for($i=0;$i<$row3['Staff'];$i++){
					echo "<i class='fa fa-star'></i>";
				}
echo"                        </dd>
                        <dt>Value</dt>
                        <dd>";
				for($i=0;$i<$row3['Value'];$i++){
					echo "<i class='fa fa-star'></i>";
				}
echo"                        </dd>
                        <dt>Atmosphere</dt>
...
?>
    ...
    <div class="p20 background-white">
        <div class="working-hours">
    		<div class="day clearfix">
    			<span class="name">Morning</span><span class="hours"><?php
																		if($morning){ echo "Available"; }
																		else { echo  "Not Available";}
																	 ?></span>
    		</div><!-- /.day -->

    		<div class="day clearfix">
    			<span class="name">Afternoon</span><span class="hours"><?php
																		if($afternoon){ echo "Available"; }
																		else { echo  "Not Available";}
																	 ?></span>
    		</div><!-- /.day -->
    		<div class="day clearfix">
    			<span class="name">Evening</span><span class="hours"><?php
																		if($evening){ echo "Available"; }
																		else { echo  "Not Available";}
																	 ?></span>
    		</div><!-- /.day -->
    	</div>
    ...
    <div class="col-sm-12">
        ...
        <form class="background-white p20 add-review" method="post" action="submitreview.php?ID=<?php echo $H_Id; ?>">
