...
<head>
    ...
    <title>hunt4place - Hotel Recommendation</title>
</head>
...
while($record=mysqli_fetch_assoc($user)){
	for($i=1;$i<=10;$i++){
		if($record["place".$i]){
			...
			$finalscore=$score/$noofuser;
			mysqli_query($con,"UPDATE food_temp3 SET score='".$finalscore."' WHERE H_ID='".$record["place".$i]."'");
		}
	}
}


$result=mysqli_query($con,"SELECT * FROM food_temp3 ORDER BY score DESC LIMIT 20 OFFSET ".$value);
$result2=mysqli_query($con,"SELECT * FROM food_temp3");
$page=mysqli_num_rows($result2)/20;
for($flag=1;$flag<=20;$flag++){
	$row=mysqli_fetch_assoc($result);
	if($row['H_ID']){
	echo '                <a href="../HotelInfo/HotelInfo.php?name='.$row["Name"].'" ><div class="col-sm-6">
                    <div class="statusbox">
                        <div class="statusbox-content">
                            <strong>'.$row["Name"].'</strong>
                            <span>'.$row["H_Address"].'</span>
...
for($count=1;$count<$page;$count++){
	$start=$count*20;
echo '<a href="HotelList.php?value='.$start.'">Page '.$count.'|</a>';
}
?>
</h2>
                                    </div>
